...
app.get('/get_session/:amount', function(req, res){
  ...
  odoo.connect(function (err, val) {
    ...
    odoo.execute_kw('orders', 'create', [[{}]], function (err, value) {
      if (err) { return res.send(err); }
      var id = value
      odoo.execute_kw('orders', 'search_read', [[[['id','=', id]],['order_id']]], function (err, value) {
        if (err) { return res.send(err); }
        ...
        fetch("https://test-gateway.mastercard.com/api/nvp/version/43", {
                ...
                .then(result => {
                  ...
                  res.send({sesionID: searchSubStr,orderID: orderID})
                })
